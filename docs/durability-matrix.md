...
The release gate remains blocked until the matrix has platform results, a reviewed directory-sync strategy, explicit cross-platform stale-lock/advisory-lock semantics, and documented backup/restore drills. Unix advisory locking is an improved baseline, not a cross-platform certification. The current MVP remains suitable only for non-critical local data where operators accept these limits.
