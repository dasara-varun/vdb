...
| Input budgets | Document size, configured maximum size, query result count, WAL record size, and JSON Lines record size are bounded | Reduces memory and denial-of-service risk; the complete working set is still in memory |
