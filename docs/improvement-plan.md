...
The repository currently contains a Rust workspace with `vdb-core` and a CLI. It implements a versioned `VDB1` file header, append-only CBOR WAL records, per-record SHA-256 integrity checks, replay recovery, optimistic document versions, bounded equality queries, equality indexes, schema and health reports, verified backups, JSON Lines export/import, single-process locking, WAL compaction, deterministic read-only Steward findings, tests, a release benchmark, and GitHub Actions checks.
...
| Done baseline | Input and replay hardening | WAL and import data are untrusted byte boundaries and can otherwise cause memory or state corruption | Stream WAL replay, bound import lines and configuration, validate record references/version sequences and supported format versions, reject common symlink paths, use private Unix modes, and refuse unsafe output overwrites | 19 core tests, strict Clippy, and documented limitations |
...
| P1 | Query planner and index lifecycle | Current equality indexes are useful but limited to simple scalar equality and still require final filtering | Add query-planner explain output, index selectivity metrics, compound/range indexes only when justified, and safe rebuild/drop workflows | Explain plans, benchmark comparisons, consistency tests under updates and compaction |
...
| P2 | OS-level process locking | The MVP lock file prevents ordinary concurrent opens but can remain after an abnormal termination and is not an advisory lock held by the operating system | Prefer a well-tested OS advisory lock abstraction or a carefully scoped stale-lock protocol; retain an explicit recovery command and never delete a live lock automatically | Multi-process tests on supported OSes and crash/stale-lock procedures |
