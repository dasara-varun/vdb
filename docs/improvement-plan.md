...
| Done baseline | Input and replay hardening | WAL and import data are untrusted byte boundaries and can otherwise cause memory or state corruption | Stream WAL replay, atomically prevalidate bounded imports, validate record references/version sequences and supported format versions, support nested equality paths consistently, reject common symlink paths, require intact backup manifests, use private Unix modes, and refuse unsafe output overwrites | 27 core tests plus 4 CLI contract tests, strict Clippy, and documented limitations |
