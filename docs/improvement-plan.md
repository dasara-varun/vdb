...
| Done baseline / P0 follow-up | Reproducible dependency supply chain | Dependency drift and known vulnerable transitive crates must be detectable | Stable Cargo generated and committed an authentic lockfile; hosted CI compares lockfile freshness, uses `--locked`, and runs RustSec auditing. Add reviewed license/source policy and SBOM generation | Authentic lockfile, green locked builds, advisory scan, documented exceptions, and release SBOM |
| P1 | Memory-bounded storage | The MVP rebuilds the complete state and indexes in memory, so database size is bounded by process memory and replay time | Retain the configurable document-count cap, then introduce byte-accounted immutable segments, checkpoints, bounded caches, and an explicit maximum database size | Large-dataset tests showing bounded RSS and bounded restart work |
