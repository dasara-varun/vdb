...
| P0 | Encryption and key management | VDB files and manifests are currently plaintext; checksums detect accidental corruption but do not provide confidentiality or authenticity | ADR-0003 proposes opt-in VDB3 AES-256-GCM storage with an external raw-key provider; accept and implement it only with explicit migration, rotation, backup handling, and secret-free diagnostics; never invent cryptography in the database layer | Accepted ADR, implementation/security review, wrong-key/tamper/rotation tests, encrypted backup/restore drills, and key lifecycle runbook |
...
The encryption work is deliberately staged behind [`ADR-0003`](decisions/0003-encrypted-storage-and-key-provider.md). The design is not an implementation approval until a maintainer accepts the ADR and the provider boundary, migration, key lifecycle, and failure behavior are independently reviewed.
