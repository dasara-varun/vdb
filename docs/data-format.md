...
Before closing and replacing the active WAL, VDB measures the compacted temporary output against the configured maximum WAL quota. If the compacted state would exceed that quota, compaction removes the temporary output, leaves the original database and active handle unchanged, and returns a quota error; a lower quota therefore never invalidates existing data. Otherwise, this prevents the previous unsafe in-place truncate-and-rewrite pattern and leaves the original path either unchanged when replacement fails or containing the compacted file after successful replacement. The exact durability of the directory-entry replacement is filesystem- and platform-dependent; see [`durability-matrix.md`](durability-matrix.md). VDB must not claim power-loss-proof compaction until directory synchronization and cross-platform crash tests are complete. Temporary files may require manual cleanup after an interrupted operation.
