...
The current VDB compaction path writes and calls `sync_all` on a same-directory temporary file, drops the active WAL handle, renames the temporary file over the database, and reopens the WAL. Normal WAL, backup, manifest, lock, and repair writes also attempt `sync_all`. On Unix, the lock file is held with rustix’s safe nonblocking advisory exclusive lock, which releases with the file descriptor when the process exits; this avoids treating a stale regular lock file as proof of a live process. The implementation retains a conservative create-new fallback on non-Unix targets. These changes are materially safer than in-place truncation, but directory metadata durability, network-filesystem behavior, and Windows/macOS locking semantics remain future hardening work. Rustix’s API is used as a reviewed safe wrapper rather than introducing raw unsafe FFI into VDB.
