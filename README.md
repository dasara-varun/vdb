...
VDB follows progressive disclosure. Beginners can use the read-only local GUI and CLI; developers can use the CLI and future authenticated APIs or SDKs; operators can inspect policies, health metrics, audit events, and recovery reports. The first successful document should not require knowledge of CBOR, WAL, MVCC, indexes, partitions, or replication.
